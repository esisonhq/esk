...
/**
 * Check the health of all database connections (primary + replicas).
 *
 * @param db - Connected database instance with replicas
 * @returns Comprehensive health check results
 */
export const checkAllConnectionsHealth = async (
  db: Database,
): Promise<{
  ...
  try {
    ...
    const primaryHealth = await checkDatabaseHealthDetailed(db);

    // If we have replicas, check them individually
    const replicaHealths: HealthCheckResult[] = [];

    // Note: In a real implementation, you'd need access to individual replica connections
    // For now, we'll simulate this since withReplicas abstracts the replica connections

    // Determine overall health
    const allHealthy =
      primaryHealth.status === 'healthy' &&
      replicaHealths.every((r) => r.status === 'healthy');
    const anyUnhealthy =
      primaryHealth.status === 'unhealthy' ||
      replicaHealths.some((r) => r.status === 'unhealthy');
    ...
    return {
      overall: {
        ...
        timestamp: new Date(),
        details: {
          primaryStatus: primaryHealth.status,
          replicaCount: replicaHealths.length,
          healthyReplicas: replicaHealths.filter((r) => r.status === 'healthy')
            .length,
        },
      },
 ...
 *
 * @param db - Database instance to check
 * @returns Promise that resolves if healthy, rejects if unhealthy
 */
export const healthCheck = async (db: Database): Promise<void> => {
...
/**
 * Create a health check middleware for express/fastify/hono.
 *
 * @param db - Database instance to check
 * @returns Health check handler function
 */
export const createHealthCheckHandler = (db: Database) => {
  return async () => {
    try {
